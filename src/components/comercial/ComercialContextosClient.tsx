...
import { initialMaleCommercialContext } from "@/lib/comercial/initial-male-commercial-context";
...
export function ComercialContextosClient({
  ...
  const [localCommercialResponses, setLocalCommercialResponses] =
    useState<CommercialResponse[]>(commercialResponses);
  const [showCreateForm, setShowCreateForm] = useState(false);
  const [form, setForm] = useState<ContextFormState>(initialFormState);
  const [isSaving, setIsSaving] = useState(false);
  const [isImportingMalePackage, setIsImportingMalePackage] = useState(false);
  ...
  async function handleImportMalePackage() {
    const confirmed = window.confirm(
      "Importar pacote inicial de atendimento masculino? Revise preços e condições antes de usar em atendimento real."
    );

    if (!confirmed) return;

    setIsImportingMalePackage(true);
    setFormError("");
    setStatusMessage("");

    try {
      const packageContext = initialMaleCommercialContext.context;
      const contextNameKey = normalizeImportKey(packageContext.name);
      const contextSlugKey = normalizeImportKey(packageContext.slug);
      const existingContext =
        localContexts.find(
          (context) =>
            normalizeImportKey(context.slug) === contextSlugKey ||
            normalizeImportKey(context.name) === contextNameKey
        ) ?? null;
      const targetContext =
        existingContext ??
        (await createCommercialContext({
          empresaId,
          data: packageContext,
        }));

      if (!existingContext) {
        setLocalContexts((current) => sortContexts([targetContext, ...current]));
      }

      const categoryBySlug = new Map(
        commercialResponseCategories.map((category) => [
          normalizeImportKey(category.slug),
          category,
        ])
      );
      const existingResponseTitles = new Set(
        localCommercialResponses
          .filter((response) => response.contextId === targetContext.id)
          .map((response) => normalizeImportKey(response.title))
      );
      const createdResponses: CommercialResponse[] = [];
      let ignoredResponses = 0;
      let missingCategories = 0;

      for (const response of initialMaleCommercialContext.responses) {
        const titleKey = normalizeImportKey(response.title);

        if (existingResponseTitles.has(titleKey)) {
          ignoredResponses += 1;
          continue;
        }
        ...
        if (!category) {
          missingCategories += 1;
          console.warn(
            `Categoria comercial não encontrada para pacote masculino: ${response.categorySlug}`
          );
        }
        ...
        const createdResponse = await createCommercialResponse({
          ...
          data: {
            ...
            internalNotes:
              "Resposta inicial do pacote Atendimento masculino — Junho. Revisar antes de uso real.",
            priority: response.priority,
          },
        });

        createdResponses.push(createdResponse);
        existingResponseTitles.add(titleKey);
      }

      if (createdResponses.length > 0) {
        setLocalCommercialResponses((current) => [
          ...createdResponses,
          ...current,
        ]);
      }
      ...
      setStatusMessage(
        [
          existingContext ? "Contexto reutilizado." : "Contexto criado.",
          `${createdResponses.length} resposta(s) criada(s).`,
          `${ignoredResponses} ignorada(s) por já existirem.`,
          missingCategories > 0
            ? `${missingCategories} resposta(s) ficaram sem categoria por slug ausente.`
            : null,
        ]
          .filter(Boolean)
          .join(" ")
      ...
      setFormError(
        error instanceof Error
          ? `Erro ao importar pacote masculino: ${error.message}`
          : "Erro ao importar pacote masculino."
      );
    } finally {
      setIsImportingMalePackage(false);
    }
  }
    ...
    <div className="flex min-h-0 flex-1 flex-col overflow-y-auto bg-[var(--bg)] p-6">
      <div className="flex flex-wrap items-start justify-between gap-4">
        ...
        <div className="flex flex-wrap gap-2">
          <button
            type="button"
            disabled={isImportingMalePackage}
            onClick={() => void handleImportMalePackage()}
            className="rounded-lg border border-[var(--border2)] bg-[var(--bg3)] px-4 py-2 text-xs font-semibold text-[var(--text2)] transition hover:bg-[var(--bg4)] hover:text-[var(--text)] disabled:cursor-not-allowed disabled:opacity-50"
          >
            {isImportingMalePackage
              ? "Importando..."
              : "Importar pacote masculino"}
          </button>
      ...
      <section className="mt-5 rounded-2xl border border-[var(--border)] bg-[var(--bg2)] p-5">
        ...
        <p className="mt-3 rounded-xl border border-[var(--border)] bg-[var(--bg3)] px-3 py-2 text-xs text-[var(--text3)]">
          O pacote masculino cria o contexto Atendimento masculino — Junho e
          respostas iniciais vinculadas a ele. Revise valores e condições antes
          de usar.
        </p>
      </section>
